...
class yurume(FrameAnimation3d):
    def __init__(self, position=(0, 10, 0), SPEED=2, jump_height=0.3, velocity=(0, 0, 0), MAXJUMP=1,
                 gravity=1, controls="wasd,left mouse click", **kwargs):
        super().__init__(name="animation\\jetpack_walking", fps=24, loop=True, autoplay=True, frame_times=120,
        ...
        # self.crosshair = Entity(model = "quad", color = color.black, parent = camera, position = (0, 0.28, 1), scale = (0.01, 0.01, 0.01))
        ...
        if x_movement != 0:
            ...
            if not xRay.hit:
                ...
                TopXRay = raycast(origin=self.world_position - (0, self.scale_y / 2 - .1, 0),
                                  direction=direction,
                                  distance=self.scale_x / 2 + math.tan(math.radians(self.slope)) * .1,
                                  ignore=[self, ])

                if not TopXRay.hit:
                    self.x += x_movement
                    HeightRay = raycast(
                        origin=self.world_position + (sign(x_movement) * self.scale_x / 2, -self.scale_y / 2, 0),
        ...
        if z_movement != 0:
            ...
            if not zRay.hit:
                ...
                TopZRay = raycast(origin=self.world_position - (0, self.scale_y / 2 - .1, 0),
                                  direction=direction,
                                  distance=self.scale_z / 2 + math.tan(math.radians(self.slope)) * .1,
                                  ignore=[self, ])

                if not TopZRay.hit:
                    self.z += z_movement
                    HeightRay = raycast(
                        origin=self.world_position + (0, -self.scale_y / 2, sign(z_movement) * self.scale_z / 2),
                        direction=(0, 1, 0), ignore=[self, ])
                    if HeightRay.hit:
                        self.y += HeightRay.distance
